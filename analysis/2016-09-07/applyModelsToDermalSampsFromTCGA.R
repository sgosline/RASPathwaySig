...
dermalData<-rna_fpkm_matrix()
normDermData<-apply(dermalData,2,zscore)
...
fullExpr<-exprData
load('../2016-08-23/mutData.Rdata')
fullMut<-mutData
#  fullMut<-getDisMutationData('') #getCcleMutationData('')
#  fullExpr<-getDisExpressionData('',getZscores=TRUE)#getCcleExpressionData('',getZscores=T)

clusterExport(cl,c("scoreNFforGene","normDermData","phenoData","mutVec",'tcga.list','fullMut','fullExpr'))
...
#for(g in genelist){
  ...
  res<-scoreNFforGene(g,datasetList,normDermData,mutVec,'dermalNfData',minPat=3,fullMut,fullExpr)
